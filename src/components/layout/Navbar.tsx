...
export function Navbar() {
  ...
  const isProductDetailPage =
    pathname.startsWith("/products/omni-browser");
  ...
  useEffect(() => {
    if (!isProductDetailPage || isDocsPage) return;
    ...
    getStars();
  }, [isProductDetailPage, isDocsPage]);
  ...
  return (
    <header className="fixed inset-x-0 top-4 z-50 flex justify-center px-4">
      ...
      >
        ...
        <ul className="hidden items-center gap-0.5 min-[1000px]:flex">
          {links.map((link) => {
            const active =
              pathname === link.href ||
              pathname.startsWith(link.href + "/");

            const isDocsLink = link.href === "/docs";
          ...
          {isProductDetailPage && (
            <li className="rounded-2xl border">
              <a
                ...
                className="flex items-center gap-2 rounded-full bg-gray-300 px-4 py-2 text-sm font-medium text-ink-muted transition-colors hover:text-ink dark:bg-black"
              >
                <svg
                  viewBox="0 0 24 24"
                  className="h-5 w-5"
        ...
        <div className="hidden shrink-0 items-center gap-2 min-[1000px]:flex">
          <ThemeToggle />
        ...
        <div className="flex items-center gap-2 min-[1000px]:hidden">
          <div className="hidden min-[270px]:block">
            <ThemeToggle />
          </div>
    ...
    className="relative z-50 flex h-9 w-9 items-center justify-center rounded-full text-ink"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="glass-strong absolute left-4 right-4 top-[4.25rem] z-40 rounded-2xl p-4 min-[1000px]:hidden"
          >
              ...
              {isProductDetailPage && (
                <li className="mt-1">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-medium text-ink-muted transition-colors hover:bg-white/5 hover:text-ink"
                  >
                    <svg
                      viewBox="0 0 24 24"
                      className="h-5 w-5"
                      ...
                      aria-hidden="true"
                    >
                      <path d="M12 .5C5.73.5.5 5.73.5 12c0 5.09 3.29 9.4 7.86 10.93.58.1.79-.25.79-.56 0-.27-.01-1.16-.02-2.11-3.2.7-3.88-1.35-3.88-1.35-.52-1.33-1.28-1.69-1.28-1.69-1.04-.71.08-.7.08-.7 1.16.08 1.77 1.19 1.77 1.19 1.03 1.76 2.7 1.25 3.36.96.1-.75.4-1.25.73-1.54-2.56-.29-5.25-1.28-5.25-5.7 0-1.26.45-2.29 1.19-3.09.74.8 1.19 1.83 1.19 3.09 0 4.43-2.69 5.41-5.26 5.7.41.36.78 1.06.78 2.14 0 1.55-.01 2.79-.01 3.17 0 .31.21.67.8.56A10.52 10.52 0 0 0 23.5 12c0-6.27-5.23-11.5-11.5-11.5z" />
                    </svg>
            ...
            <div className="mt-3 border-t border-base-border px-2 pt-4">
              <Button
                href="/contact#support"
                size="sm"
